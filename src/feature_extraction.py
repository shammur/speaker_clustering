...
def feat_extract(filelist,feat_type,n_fft_length=512,hop=160,vad=True,cmvn=False,exclude_short=500,seg_windows=False,dim=40,win_len=400):
    ...
    for index,wavname in enumerate(tqdm(filelist)):
        ...
        if feat_type =='melspec':
            Y = librosa.feature.melspectrogram(y,sr,n_fft=n_fft_length,hop_length=hop,n_mels=dim,fmin=133,fmax=6955)
        elif feat_type =='mfcc':
            Y = librosa.feature.mfcc(y,sr,n_fft=n_fft_length,hop_length=hop,n_mfcc=dim,fmin=133,fmax=6955)
        elif feat_type =='spec':
            Y = np.abs( librosa.core.stft(y,n_fft=n_fft_length,hop_length=hop,win_length=win_len) )
        elif feat_type =='logspec':
            Y = np.log( np.abs( librosa.core.stft(y,n_fft=n_fft_length,hop_length=hop,win_length=win_len) ) )
        elif feat_type =='plspec': #power-law compression
            Y = np.power( np.abs( librosa.core.stft(y,n_fft=n_fft_length,hop_length=hop,win_length=win_len) ),0.3 )
        elif feat_type =='plspec_real': #power-law compression
            Y_original = librosa.core.stft(y,n_fft=n_fft_length,hop_length=hop,win_length=win_len)
            ...
            Y = np.log( librosa.feature.melspectrogram(y,sr,n_fft=n_fft_length,hop_length=hop,n_mels=dim,fmin=133,fmax=6955)+epsilon )
...
#            break
    ...
    tffilename = feat_type+'_win'+str(win_len)+'_hop'+str(hop)
    if vad:
        tffilename += '_vad'
    if cmvn=='m':
        tffilename += '_cmn'
