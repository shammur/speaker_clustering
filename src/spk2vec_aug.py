...
class nn:
    ...
    def __init__(self, x1, y_, y_string, shapes_batch, softmax_num,is_training,input_dim, is_batchnorm):
        ...
        with tf.variable_scope("softmax") as scope:
            ...
            self.end_points = self.net(x1, shapes_batch, softmax_num,is_training,input_dim,is_batchnorm)

            # Create loss
            # self.loss    = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=y_, logits=self.o1),name="loss_mean")
            self.loss    = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=y_, logits=self.end_points['predictions']),name="loss_mean")
        self.label=y_
